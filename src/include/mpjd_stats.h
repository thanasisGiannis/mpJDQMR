...
namespace mpjd {
struct statistics {
    int m_matVecs        = 0; // number of matrix-vector 
                              // (block matVec is assumed as 
                              // block dimension matVecs)
...
struct statistics mpjdStatistics::stats;  // in order to be shared
...
class basisStatistics : public mpjdStatistics {
  public:
    void updateRestarts(int counter) { 
      (this->stats).m_numRestarts+=counter;
    }
    void updateOrthogonalizations(int counter){ 
      (this->stats).m_numOrth+=counter;
    }
    void updateNumInnerLoop(int counter){ 
      (this->stats).m_innerLoopIters+=counter;
    }
};
